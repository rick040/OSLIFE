 ...
 * Claude Code, claude.ai) from the standalone `oslife-memory` MCP server
 * (integrations/claude-mcp/) and logs it straight into OSLIFE's memory —
 * same destination as a Braindump capture (`braindump_entries`), so it shows
 * up in the Capture grid, feeds `search_memory()`'s hybrid recall, mirrors
 ...
 * the MCP tool call already IS Claude, mid-conversation, producing a
 * pre-digested summary/key-points/tags, so a second round-trip would just be
 ...
 *               "sourceUrl"?: string, "conversationTitle"?: string,
 *               "insight"?: { "category"?: string, "takeaway": string, "application": string } }
 *   response: { "ok": true, "id": "<uuid>", "status": "ready"|"duplicate" }
 *
 * Deploy:
...
Deno.serve(async (req) => {
  ...
  const { data: row, error: insertErr } = await sb
    ...
    .insert({
      ...
      meta: { conversationTitle, ingestedVia: "claude-mcp" },
    })
    .select("id")
    .single();
  ...
  // Best-effort enrichment, in-process with the service-role client — there's
  // no user JWT here to thread through embed-memory/materialize-note the way
  // braindump-ingest does, so this mirrors embed-memory-backfill's approach
  // instead: call the shared Voyage/vault/cognee helpers directly.
  ...
  cogneeRemember(`[claude-gesprek]\n${markdown}`).catch(() => {});

  // Optional: Claude itself flagged a reusable idea/insight worth a spot in
  // the Kennisbank — same suggested-status shape braindump-ingest writes.
