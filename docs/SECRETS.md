...
| `YOUTUBE_COOKIE_HEADER` *(optioneel, maar in de praktijk nodig)* | braindump-ingest | Een `naam=waarde; naam2=waarde2; ...` Cookie-headerstring van een echte, ingelogde YouTube-sessie. **Correctie op een eerdere aanname**: braindump-ingest's eigen caption-fetch (`_shared/youtube.ts`, geen yt-dlp) leek "geen cookies nodig" te hebben, maar een onbevestigd verzoek krijgt in de praktijk vrijwel altijd YouTube's `playabilityStatus: LOGIN_REQUIRED` ("log in om te bevestigen dat je geen bot bent") terug — dezelfde bot-check die yt-dlp ook raakt, alleen op HTTP-niveau i.p.v. via yt-dlp. Zonder deze header werkte deze route in de praktijk dus 0x; elke echte transcript kwam alsnog via de worker. Afleiden uit dezelfde `cookies.txt` die de worker gebruikt: `sed -e 's/^#HttpOnly_//' cookies.txt \| awk -F'\t' '!/^#/ && NF==7 {printf "%s=%s; ", $6, $7}'` (Netscape-formaat, kolommen 6/7 zijn naam/waarde; de `sed` ontdoet HttpOnly-cookies van hun `#HttpOnly_`-prefix zodat ze niet als commentaarregel worden overgeslagen). Zonder deze secret valt braindump-ingest terug op vandaag's grotendeels-non-functionele gedrag voor YouTube-transcripts (metadata-only, tenzij de worker het overneemt). |
| `INSTAGRAM_COOKIE_HEADER` *(optioneel)* | braindump-ingest | Zelfde probleem/fix als `YOUTUBE_COOKIE_HEADER`, maar voor Instagram: een onbevestigd verzoek naar een post krijgt in de praktijk steeds vaker Instagram's inlogmuur terug (geen `og:image`/`og:description` op de pagina) i.p.v. de echte post — dit is de "Error: no og:image found" die je in de Braindump-kaart kan zien. Zelfde Cookie-headerstring, afgeleid van een `cookies.txt` export terwijl je bent ingelogd op instagram.com. Alleen gebruikt door `processSocial()` (het pad zonder worker, of als fallback als de worker onbereikbaar is) — zie ook `IG_COOKIES_PATH` hieronder voor de worker zelf. Zonder deze secret/cookies valt Instagram terug op een titel/bijschrift-only notitie i.p.v. de volledige post-beschrijving. |
| `BRAINDUMP_WORKER_URL` *(optioneel)* | braindump-ingest | Publieke URL van de braindump media-worker (`integrations/braindump-worker/`). Blijft de fallback voor video's zonder ondertiteling (ook met `YOUTUBE_COOKIE_HEADER` gezet) en voor losse video/audio-bestanden. Zonder deze URL valt media terug op metadata-only (oEmbed/OpenGraph) — de app breekt nooit zonder. |
...
| `CLAUDE_INGEST_SECRET` | claude-chat-ingest | **zelf verzinnen** (random) — gedeeld met de `oslife-memory` MCP-server (`integrations/claude-mcp/`), niet met de telefoon/MacroDroid-secrets. Zonder deze secret weigert claude-chat-ingest elk verzoek (fail closed). |
...
| Claude-gesprekken → geheugen (claude-chat-ingest + `oslife-memory` MCP) | `CLAUDE_INGEST_SECRET`, `OSLIFE_USER_ID` — zie `integrations/claude-mcp/README.md` |
