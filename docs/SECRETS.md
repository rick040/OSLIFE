...
| `BRAINDUMP_WORKER_URL` *(optioneel)* | braindump-ingest | Publieke URL van de braindump media-worker (`integrations/braindump-worker/`). YouTube heeft deze niet meer nodig voor het transcript — braindump-ingest haalt captions zelf gratis op via YouTube's eigen ondertitel-tracks (zie `supabase/functions/_shared/youtube.ts`); alleen video's zonder enige ondertiteling, en losse video/audio-bestanden, vallen nog terug op deze worker. Zonder deze URL valt media (video/audio, en YouTube zonder captions) terug op metadata-only (oEmbed/OpenGraph) — de app breekt nooit zonder. |
