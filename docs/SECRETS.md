...
| `SUPADATA_API_KEY` *(optioneel, maar primair pad)* | braindump-ingest, braindump-worker | supadata.ai dashboard → API keys. **Vervangt** de eerdere `YOUTUBE_COOKIE_HEADER`-aanpak volledig — die bleek dezelfde `playabilityStatus: LOGIN_REQUIRED` bot-check te raken als yt-dlp (geverifieerd met een live curl-test: ook videoDetails/description bleken achter diezelfde muur te zitten, niet alleen captions), dus was op een echte, verlopende browsersessie aangewezen — een onderhoudslast zonder einde. Supadata is een derde partij die bot-check/cookie-rotatie/Whisper-fallback al als hun hele business heeft opgelost: één API-call geeft transcript + volledige metadata (titel/beschrijving/kanaal/thumbnail/duur) terug, ook voor video's zonder ondertiteling (eigen Whisper-fallback). Gratis tier: 100 requests/maand, geen kaart nodig — bij een single-user app realistisch dekkend. Zonder deze key valt braindump-ingest terug op gratis oEmbed (titel/kanaal/thumbnail, geen beschrijving/transcript) en/of de worker's yt-dlp+cookies-pad hieronder — de app breekt nooit zonder. Zelfde waarde in beide plekken instellen (Supabase-secret + de worker's eigen `.env`). |
| `BRAINDUMP_WORKER_URL` *(optioneel)* | braindump-ingest | Publieke URL van de braindump media-worker (`integrations/braindump-worker/`). Nu de laatste-redmiddel-fallback: alleen bereikt voor video's die Supadata niet kon transcriberen (geen key, gratis tier op, of een lange video met een async job die de edge function niet kan pollen) en voor losse video/audio-bestanden. Zonder deze URL valt media terug op metadata-only (oEmbed/OpenGraph) — de app breekt nooit zonder. |
...
| Braindump · YouTube-transcript + metadata (inline, geen worker) | `SUPADATA_API_KEY` (optioneel maar primair pad — gratis tier 100/maand; zonder valt terug op gratis oEmbed, geen transcript) |
| Braindump media-worker (laatste-redmiddel: video/audio transcriptie, YouTube dat Supadata niet kon) | `BRAINDUMP_WORKER_URL`, `WORKER_SECRET` (beide optioneel — zonder valt media terug op metadata-only), zelfde `SUPADATA_API_KEY` (optioneel, herprobeert eerst) |
