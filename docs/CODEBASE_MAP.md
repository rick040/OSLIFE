...
| `notify-tick` | cron every 5 min (bearer `CRON_SECRET`, no JWT) | Composes/sends proactive Telegram nudges (briefing, check-in, habit reminders, urgent alerts, inference digest, plus random throughout-the-day nudges — task/habit, keep-me-sharp, suggestion, ad-hoc check-in — gated by `notification_prefs.nudges_enabled`). |
