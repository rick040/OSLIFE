...
| Profile (Profiel) | `src/views/Profile.tsx` | `profile` | Identity/personality profile screen, 3 tabs over `store.identityProfile`, all built on the same concrete/structured shape (category key → short discrete items, never narrative) from `src/profile.ts`'s canonical `PERSONA_CATEGORIES` (toolstack/habits/strengths/weaknesses/interests/character/workstyle/communication/accelerators) and `LANDSCAPE_CATEGORIES` (people/habits/time/money/balance/focus/environment) — every category is also manually editable (add/remove item) via the single `setProfileCategoryItems()` action. **Huidig** — AI-synthesized current-state read from learned facts, patterns, profile facts, braindumps and habits (`generateCurrentProfile()` → `heyra/identity.ts:synthesizeCurrentProfile`, brain-first with rule-based fallback, same honesty contract as `goals.ts`). **Droom** — `dreamNotes` is Rick's own free-form text (a self-interview, never AI-generated, preserved verbatim); `distillDreamProfile()` → `heyra/identity.ts:synthesizeDreamProfile` extracts it into the same category shape as Huidig (brain-only, no rule fallback — free prose can't be bucketed reliably), still hand-editable afterward. **Landschap** — AI-synthesized environment bridging current → dream (`generateLandscape()` → `heyra/identity.ts:synthesizeLandscape`), gated on some dream signal (structured categories or raw notes). Persisted to the one-row `identity_profile` table (`current`/`dream`/`landscape` jsonb, `dream_notes` text). |
...
| StrategieHQ | `src/views/StrategieHQ.tsx` | `strategiehq` | Business-ideas hub: capture (voice/text) → `idea-elaborate` edge function produces a full strategic write-up (feasibility, milestones, financials, SWOT). Second, opt-in pipeline per idea: "Genereer MVP Launch Plan" → `idea-mvp-plan` edge function produces a lean-startup validation plan (channels, cheap experiments, phased roadmap, signals to watch, explicit caveat about cold email's low reply rate) so Rick can test real interest before building anything. No external source — fully native (`business_ideas` table). |
...
- **`identity.ts`** — Profile screen synthesizer. `synthesizeCurrentProfile(ctx)` distills learned facts/patterns/profile facts/braindumps/habits into a structured `IdentitySnapshot` (category key → short discrete items, see `src/profile.ts`'s `PERSONA_CATEGORIES`); falls back to a rule-based bucketing of live patterns/facts/habits on brain failure. `synthesizeDreamProfile(dreamNotes)` distills Rick's own free-form dream text into the same category shape — brain-only, no rule fallback (free prose can't be bucketed reliably), `null` on failure. `synthesizeLandscape(ctx)` proposes the `Landscape` (`LANDSCAPE_CATEGORIES`: people/habits/time/money/balance/focus/environment) bridging current → dream — requires some dream signal, `null` otherwise.
...
- **`src/profile.ts`** — canonical category definitions for the Profile screen: `PERSONA_CATEGORIES` (current + dream persona, same shape so they're directly comparable) and `LANDSCAPE_CATEGORIES`; `emptySnapshot()`/`emptyLandscape()`/`hasAnyItems()`. Single source of truth consumed by `heyra/identity.ts` (prompt/schema keys), `store.ts` (defaults) and `views/Profile.tsx` (render order/labels).
...
| 31 | `20260727090000_identity_profile_categories.sql` | Restructures `identity_profile` from narrative summaries to concrete categories (`{"categories": {key: string[]}, "generatedAt"}` for `current`/`dream`/`landscape` — see `src/profile.ts`). Renames `dream_md` → `dream_notes` (preserves Rick's existing hand-written dream text verbatim) and adds a `dream` jsonb column distilled from it; `current`/`landscape` are fully regenerable on demand so they're reset to the new empty shape rather than remapped field-by-field. |
