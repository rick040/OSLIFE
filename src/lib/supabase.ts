...
export async function fetchTasks(): Promise<Thread[]> {
  return fetchRows(
    'tasks',
    'id,domain,title,owed_to,due,status,priority,notes,checklist,focus_date,created_at',
    ...
    (r) => ({
      ...
      focusDate: (r.focus_date as string) ?? null,
    }),
  )
}
...
function classifyApp(name: string): 'work' | 'social' | 'media' | 'comms' | 'other' {
  ...
  if (/docs|sheets|slides|word|excel|powerpoint|notion|figma|canva|code|github|gitlab|slack|drive|calendar|agenda|jira|linear|vscode|xcode|terminal|claude|fiverr/.test(n)) return 'work'
  return 'other'
}
