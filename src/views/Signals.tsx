...
export default function Signals() {
  ...
  const screenToday = screenDays.find((s) => s.date === TODAY) ?? screenDays[screenDays.length - 1]
  const locToday = locationDays.find((l) => l.date === TODAY) ?? locationDays[locationDays.length - 1]
  const musicToday = musicDays.find((m) => m.date === TODAY) ?? musicDays[musicDays.length - 1]
  ...
  const locTotal = locToday.timeHome + locToday.timeOut + locToday.timeCommute || 1
        ...
        <div className="space-y-1.5">
          {topPlaces.map((p) => (
            <div key={p.name} className="flex items-center gap-2 text-sm">
              ...
              <div className="h-1.5 flex-1 rounded-full bg-line overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${Math.min(100, (p.minutes / topPlaces[0].minutes) * 100)}%`, background: DOMAIN_HEX[p.domain] }} />
              </div>
