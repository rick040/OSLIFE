 ...
 *
 * Projects/Clients are managed natively in the app (no external sync) — this
 * project no longer touches Notion.
 *
 * SETUP
 ...
 *      FIVERR_TEMPLATE_DOC_ID     1bAR-iexpTYoDcPgWlW7aAVXMnXEjE9A8QVmHBe-09oc (or your own copy)
 *      FIVERR_DOC_WEBHOOK_SECRET  <random string — also set as the fiverr-process-intake
 *                                  edge function secret FIVERR_DOC_WEBHOOK_SECRET>
 ...
 *
 * 3. For the Fiverr proposal-doc generator (doPost below, used by the
 *    fiverr-process-intake edge function — see docs/README-fiverr-intake.md):
 *    Deploy → New deployment → type "Web app" → execute as "Me", who has
 *    access "Anyone". Copy the resulting /exec URL into the edge function's
 *    FIVERR_DOC_WEBHOOK_URL secret. Re-deploy (new version) after any edit to
 *    doPost/createProposalDoc_ — Web app deployments pin a code version.
 *
 ...
 *   `conflict`) are left untouched instead of overwritten. Unused now that
 *   projects/clients are native-only, kept generic for any future upsert that
 *   must never clobber an in-app edit.
 */
...
// ── 4. FIVERR INTAKE — proposal/invoice doc generator (Web App) ─────────────
// Called by the fiverr-process-intake Supabase edge function (server-side —
// see docs/README-fiverr-intake.md). Copies the PRJCT invoice/proposal
// template Google Doc and fills it in for however many deliverable packages
// the drafted project actually has (the template ships with 3 service rows;
// rows are added or removed to match). Runs under Rick's own Google account
// via DocumentApp/DriveApp — no service account, no Google Cloud project.
//
// Deploy this file as a Web App (see SETUP step 3 above) and put its /exec
// URL + a shared secret into the edge function's FIVERR_DOC_WEBHOOK_URL /
// FIVERR_DOC_WEBHOOK_SECRET secrets.
function doPost(e) {
  ...
  var expected = prop('FIVERR_DOC_WEBHOOK_SECRET');
  if (!expected || payload.secret !== expected) {
    return out({ error: 'unauthorized' });
  }
  try {
    var url = createProposalDoc_(payload);
    return out({ ok: true, docUrl: url });
  } catch (err) {
    log('createProposalDoc_ failed: ' + (err && err.message));
    return out({ error: String(err && err.message || err) });
  }
}

/**
 * payload: { clientName, projectTitle, packages: [{name, specs[], qty,
 *   unitPrice, total}], subtotal, total, invoiceNumber?, dateToday }
 * Returns the new Doc's URL. Leaves the template's Knab payment link
 * untouched — it's a single-use betaalverzoek link tied to a specific
 * amount, so it must be regenerated by hand per invoice before sending.
 */
function createProposalDoc_(payload) {
  var templateId = prop('FIVERR_TEMPLATE_DOC_ID');
  if (!templateId) throw new Error('Set FIVERR_TEMPLATE_DOC_ID script property.');

  var name = 'Voorstel - ' + (payload.projectTitle || 'Project') + ' - ' + (payload.clientName || 'Client');
  var copy = DriveApp.getFileById(templateId).makeCopy(name);
  var doc = DocumentApp.openById(copy.getId());
  var body = doc.getBody();

  var simple = {
    '{DateToday}': payload.dateToday || Utilities.formatDate(new Date(), 'Europe/Amsterdam', 'dd-MM-yyyy'),
    '{Bedrijfsnaam}': payload.clientName || '',
    '{StraatnaamNr}': '',
    '{PostcodePlaats}': '',
    '{KvkNr}': '',
    '{FactuurNr}': payload.invoiceNumber || '',
    '{ProjectOmschrijving}': payload.projectTitle || '',
    '{BetalenVoor}': '50% aanbetaling, 50% bij levering',
    '{Sub}': Number(payload.subtotal || 0).toFixed(2),
    '{Total}': Number(payload.total || 0).toFixed(2),
  };
  Object.keys(simple).forEach(function (key) {
    body.replaceText(escapeRegexLiteral_(key), simple[key]);
  });

  fillServiceRows_(body, payload.packages || []);
...
/**
 * The template's 2nd table is the services table with a fixed 3 rows
 * (Dienst1/2/3) between the header row and the Opmerkingen/Subtotaal/Totaal
 * rows. Adds rows (copying the last template row) for >3 packages, removes
 * unused ones for <3, then overwrites each row's cells directly — simpler
 * and more robust than per-token replaceText for the variable specs list.
 */
function fillServiceRows_(body, packages) {
  var tables = body.getTables();
  var svcTable = tables[1];
  if (!svcTable) throw new Error('Could not find the services table (expected 2nd table in the doc).');

  var FIRST_SERVICE_ROW = 2; // 0=blank row, 1=header row, 2.. = Dienst rows
  var TEMPLATE_SERVICE_ROWS = 3;
  var count = Math.max(packages.length, 1);

  for (var i = TEMPLATE_SERVICE_ROWS; i < count; i++) {
    var templateRow = svcTable.getRow(FIRST_SERVICE_ROW + TEMPLATE_SERVICE_ROWS - 1).copy();
    svcTable.insertTableRow(FIRST_SERVICE_ROW + i, templateRow);
  }
  for (var j = TEMPLATE_SERVICE_ROWS - 1; j >= count; j--) {
    svcTable.removeRow(FIRST_SERVICE_ROW + j);
  }

  for (var r = 0; r < count; r++) {
    var pkg = packages[r] || { name: '', specs: [], qty: 0, unitPrice: 0, total: 0 };
    var row = svcTable.getRow(FIRST_SERVICE_ROW + r);
    var descLines = [pkg.name || ''].concat((pkg.specs || []).map(function (s) { return '  - ' + s; }));
    setCellLines_(row.getCell(0), descLines);
    row.getCell(1).setText(pkg.qty != null ? String(pkg.qty) : '');
    row.getCell(2).setText('€' + Number(pkg.unitPrice || 0).toFixed(2));
    var total = pkg.total != null ? pkg.total : (pkg.qty || 0) * (pkg.unitPrice || 0);
    row.getCell(3).setText('€' + Number(total).toFixed(2));
  }
}
...
function setCellLines_(cell, lines) {
  ...
  lines.forEach(function (line, i) {
    if (i === 0) cell.getChild(0).asParagraph().setText(line);
    else cell.appendParagraph(line);
  });
}
