 ...
 * for steps and posts to the same endpoint, so the "Stappen" tab below is no
 * longer read at all (Tasker is the sole steps source). Tailored to the
...
// Steps are deliberately NOT read from the "Stappen" tab anymore — Tasker
// (reading Health Connect directly) is now the sole steps source, so this
// row omits `steps` entirely and health-ingest leaves that column untouched
// instead of overwriting Tasker's value with a stale/zero Sheet number.
