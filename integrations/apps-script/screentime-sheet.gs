 ...
 *   Tab "Ontgrendelingen"        ← skipped — pickups now come from phone_events
 *                                   (MacroDroid, real-time) via phone-events-ingest's
 *                                   refreshPickups(), not from this sheet anymore.
 *
...
function syncScreentimeSheet() {
  ...
  try {
    ...
    log('syncScreentimeSheet: ' + rows.length + ' app-rows (upserted ' + total + ')');
  } finally {
    lock.releaseLock();
  }
}
